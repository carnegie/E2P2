...
class Predictions(object):
	...
	def generate_priam_predictions(self, path_to_priam_weight, path_to_sequence_ecs, logging_level, logger_name):
		...
		try:
			...
			with open(path_to_sequence_ecs, 'r') as ptse:
	...
	def generate_deepgoplus_predictions(self, path_to_deepgoplus_weight, path_to_results_tsv, cuttoff, logging_level, logger_name):
		self.read_weight_file(path_to_deepgoplus_weight, logging_level, logger_name)
		logger = logging.getLogger(logger_name)
		try:
			try:
				logger.log(prog.logging_levels[logging_level],
						   "Reading results.tsv: \"" + path_to_results_tsv + "\"")
			except KeyError:
				logger.log(logging.DEBUG, "Reading results.tsv: \"" + path_to_results_tsv + "\"")
			with open(path_to_results_tsv, 'r') as ptrt:
				for line in ptrt:
					line = line.rstrip()
					info = line.split('\t')
					try:
						query_id = info[0]
						self.seq_list.add(query_id)
						deepgoplus_results = [res.split('|') for res in info[1:] if float(res.split('|')[1]) >= cuttoff]
						if len(deepgoplus_results) > 0:
							try:
								for d_res in deepgoplus_results:
									try:
										if d_res[1] > self.predictions[query_id][d_res[0]]:
											self.predictions[query_id][d_res[0]] = d_res[1]
									except KeyError:
										self.predictions[query_id].setdefault(d_res[0], d_res[1])
							except KeyError:
								self.predictions.setdefault(query_id, {})
								for d_res in deepgoplus_results:
									cur_d_score = self.predictions[query_id].setdefault(d_res[0], d_res[1])
									if d_res[1] > cur_d_score:
										self.predictions[query_id][d_res[0]] = d_res[1]
						else:
							self.predictions.setdefault(query_id, {})
					except IndexError:
			...
			logger.log(logging.ERROR, "results.tsv not found: \"" + path_to_results_tsv + "\"")
			sys.exit(1)
